...
public static class Poly2Mesh
{
	...
	public static Mesh CreateMesh(Polygon polygon)
    {
		...
		try {
            ...
            Debug.LogWarning("Error on expert! Error in triangalation");
			throw e;
		}
    ...
    public static List<int> ReturnTriangles(ref List<Vector3> points, Vector3 dir)
    {
        Polygon polygon = new Polygon();
        polygon.outside = points;
        polygon.CalcPlaneNormal(dir);
        Mesh mesh = CreateMesh(polygon);
        points = mesh.vertices.OfType<Vector3>().ToList();
        return mesh.triangles.OfType<int>().ToList();
    }
